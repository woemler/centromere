...
import com.blueprint.centromere.core.dataimport.cli.manifest.ImportManifest;
import com.blueprint.centromere.core.dataimport.cli.manifest.ManifestFile;
...
public class ManifestParsingTests {
	...
	private void testManifestObject(ImportManifest manifest){
		...
		Assert.isTrue(manifest.getParameters().containsKey("skip-invalid-records"));
		Assert.isTrue("true".equals(manifest.getParameters().get("skip-invalid-records")));
