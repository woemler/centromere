...
public class BasicColumnMappingRecordReader<T extends Model<?>>
    ...
    private static final Logger LOGGER
        = LoggerFactory.getLogger(BasicColumnMappingRecordReader.class);
    ...
    /**
     * See {@link DelimitedTextFileRecordReader#readRecord()}.
     *
     * @return model record
     */
    @Override
    ...
    /**
     * Extracts the column headers from a file and attempts to match them to {@link Model} fields.
     * If a header cannot be matched to a field, this column will be ignored.
     *
    ...
    private String getMatchedHeaderFieldName(String headerName) {
        ...
        for (Map.Entry<String, String> entry : fieldNameMap.entrySet()) {
            if (headerName.toLowerCase().replaceAll("[\\W\\s_-]", "")
    ...
    /**
     * Parses a single line of text and maps the values to the appropriate {@link Model} field
     * values. Empty column values will be mapped as null values.  Throws an exception if the string
     * value cannot be converted to the appropriate type.
     *
    ...
    /**
     * Converts a string value from a parsed file to the appropriate type required for the {@link
     * Model} field.  Throws an exception if conversion cannot be applied.
     *
    ...
    private Object convertFieldValue(String s, Class<?> type) throws DataProcessingException {
        ...
        if (conversionService.canConvert(String.class, type)) {
            ...
            throw new DataProcessingException(String.format("Cannot convert String type to %s.",
                type.getName()));
        }
    }

    /**
     * See {@link DelimitedTextFileRecordReader#isSkippableLine(List)}.
     */
    @Override
