...
public class GctTranscriptExpressionProcessor<T extends TranscriptExpression<ID>, ID extends Serializable> 
    extends GenericRecordProcessor<T> {

  public GctTranscriptExpressionProcessor(
      Class<T> model,
      GeneRepository geneRepository, 
      TranscriptExpressionRepository<T, ID> repository,
      SampleRepository sampleRepository,
      DataImportProperties dataImportProperties
  ) {
    this.setModel(model);
    this.setReader(new GctTranscriptExpressionFileReader<>(model, geneRepository, sampleRepository, dataImportProperties));
