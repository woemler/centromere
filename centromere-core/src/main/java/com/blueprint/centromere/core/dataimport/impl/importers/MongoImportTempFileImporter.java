...
package com.blueprint.centromere.core.dataimport.impl.importers;
...
public class MongoImportTempFileImporter<T extends Model<?>> extends AbstractFileImporter<T> {
  ...
  public MongoImportTempFileImporter() {
  }
  ...
  private String buildImportCommand(String filePath){
    ...
    if (this.getEnvironment().containsProperty(ApplicationProperties.DB_USER)) {
      sb.append(String.format(" --username %s ", this.getEnvironment().getRequiredProperty(ApplicationProperties.DB_USER)));
    }
    if (this.getEnvironment().containsProperty(ApplicationProperties.DB_PASSWORD)){
      sb.append(String.format(" --password %s ", this.getEnvironment().getRequiredProperty(ApplicationProperties.DB_PASSWORD)));
    }
    if (this.getEnvironment().containsProperty(ApplicationProperties.DB_HOST)) {
      if (this.getEnvironment().getRequiredProperty(ApplicationProperties.DB_HOST).contains(":")) {
        sb.append(String.format(" --host %s ", this.getEnvironment().getRequiredProperty(ApplicationProperties.DB_HOST)));
      } else if (this.getEnvironment().containsProperty(ApplicationProperties.DB_PORT)) {
        sb.append(String.format(" --host %s:%s ", this.getEnvironment().getRequiredProperty(ApplicationProperties.DB_HOST), 
            this.getEnvironment().getRequiredProperty(ApplicationProperties.DB_PORT)));
      } else {
        sb.append(String.format(" --host %s:27017 ", this.getEnvironment().getRequiredProperty(ApplicationProperties.DB_HOST)));
      }
    }
    sb.append(String.format(" --db %s ", this.getEnvironment().getRequiredProperty(ApplicationProperties.DB_NAME)));
