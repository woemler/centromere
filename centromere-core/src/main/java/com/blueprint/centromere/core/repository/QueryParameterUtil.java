...
public final class QueryParameterUtil {
    ...
    public static Map<String, QueryParameterDescriptor> getAvailableQueryParameters(
        ...
        LOGGER.debug(String.format("Determining available query options for model: %s",
            model.getName()));
        Class<?> current = model;
        ...
        while (current.getSuperclass() != null) {
            ...
            for (Field field : current.getDeclaredFields()) {
                ...
                LOGGER.debug(String.format("Inspecting field %s for model: %s",
        ...
        LOGGER.debug(String.format("Found %d query options for model: %s",
            paramMap.size(), model.getName()));
        return paramMap;
    }
    ...
    /**
     * See {@link QueryParameterUtil#convertParameter(Object, Class, ConversionService)}.
     */
    private static Object convertParameter(Object param, Class<?> type) {
