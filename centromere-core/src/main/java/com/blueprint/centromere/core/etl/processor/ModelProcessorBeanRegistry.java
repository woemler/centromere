 ...
 * @since 0.5.0
 */
public class ModelProcessorBeanRegistry<T extends DataProcessor<?>>
    ...
    void registerBean(T component) {
        ...
        if (!processors.contains(component)) {
            ...
            LOGGER.warn(String.format("DataProcessor already registered: %s",
                component.getClass().getName()));
        }
    }
     ...
     * @return the same input bean, unmodified
     * @throws BeansException will never be thrown here
     */
    @Override
     ...
     *
     * @return all registered data types
     */
    @Override
