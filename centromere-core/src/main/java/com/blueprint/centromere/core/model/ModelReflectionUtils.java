...
public final class ModelReflectionUtils {
     ...
     */
    public static List<String> getPersistableNonEntityFieldNames(
        Class<?> model, List<String> ignoredFields) {
        List<String> columns = new ArrayList<>();
        Class<?> currentClass = model;
        while (currentClass.getSuperclass() != null) {
            for (Field field : currentClass.getDeclaredFields()) {
                if (ModelReflectionUtils.isPersistableNonEntityField(field, ignoredFields)) {
